...
Se comprueba que efectivamente existen diferencias significatuvas respecto de los supervivientes en función del númreo de hermanos.

Si ahora se realiza el mismo analisis pero para el número de padres e hijos:
...
Merece la pena observar si existe cierta correlación entre estas variables, por lo que ser verifica a traves del metodo de *Spearman* al ser las distribuciones no normales:
...
De manera que se puede observar directamente como existe una mayor proporción de mujeres que sobrevivieron, y a su vez de niños, como de pasajeros de primera clase.

Ahora se va a realizar un modelo supervisado para ver si los datos que se han podido ver arriba tienen alguna interpretación adicional. Para ello se plantea un modelo basado en arboles de decisión:
...
arbol <- C50::C5.0(datos_arbol, datos$Survived, rules=TRUE)
summary(arbol)
arbol <- C50::C5.0(datos_arbol, datos$Survived)
plot(arbol)
```

En el modelo, al tener en consideración de manera conjunta todas las variables se observa como realiza el modelo basándose en tan solo dos variables, por lo que es capaz de inferir si la persona sobrevivirá a través solo de la edad y el grupo de edad.
...
```{r, message=FALSE, warning=FALSE}
modelo_reglo <- glm(datos$Survived ~ GrupoEdad+Pclass+Sex,data=datos_arbol,
                    family="binomial")
summary(modelo_reglo)
```

Ahora se puede observar como la significancia de las variables realmente cae con el grupo de edad de los ancianos, siendo alta para el resto de las variables, lo que contraste con el resultado anterior. Si se calcula la efectividad del modelo construido aún con los datos usados, sin tener en cuenta que se podría haber generado un conjunto de entrenamiento y otro de test:


```{r, message=FALSE, warning=FALSE}
prediccionRL <- predict(modelo_reglo, datos_arbol,
                        type="response")
prediccionRL[prediccionRL > 0.5]  <- "Si"
prediccionRL[prediccionRL <= 0.5]  <- "No"
prediccionRL <- as.factor(prediccionRL)
efectvidad <- sum(prediccionRL == datos$Survived) / length(datos$Survived)
efectvidad
```

Se observa que casi no presenta diferencia con el árbol, por lo que las predicciones serán parecidas.
...
Después de haber modificado los parametros para que solo queden dos grupos como los que se buscan y teniendo en cuenta que los grupos no corresponden entre cero y No, ya que *DBSCAN* no nombra los grupos, suponiendo el mejor caso, se tiene que el número de muestras correctamente clasificadas es del 65%, sensiblemente inferior a los metodos anteriores.


## Resolución del problema. A partir de los resultados obtenidos, ¿cuáles son las conclusiones? ¿Los resultados permiten responder al problema?

A través del análisis realizado se ha podido encontrar unos resultados que respondan el planteamiento original del problema, ya que se puede dar explicación a la mayor supervivencia a partir de las distintas caracteristicas de los pasajeros.

Así se puede concluir, que el protocolo establecido de las mujeres y los niños primero, se cumple en este caso ya que correla ampliamente el nivel de supervivencia de estas clases. Además se puede observar como también existe un mayor indice de supervivencia entre los pasajeros de las clases superiores que de la inferiores.


Por último, se genera el fichero con los datos empleados para que pueda ser usado posteriormente:

```{r, message=FALSE, warning=FALSE}
write.csv(datos, file = "data/Titanic_clean.csv", row.names=FALSE)
```
